...
function App() {
  const { data, isLoading, error } = useQuery<CommentWithId[]>(
    ['comments'], // <-- key de la query
    getComments
  )
  const queryClient = useQueryClient()

  const { mutate, isLoading: isLoadingMutation } = useMutation({
    mutationFn: postComment, // <-- funcion que se ejecuta para hacer la mutacion
    onMutate: async (newComment) => {
      // cancelar todas las queries de comments
    ...
    onError: (error, variables, context) => {
      console.error(error)
      // volver a poner los comentarios anteriores en caso de error
